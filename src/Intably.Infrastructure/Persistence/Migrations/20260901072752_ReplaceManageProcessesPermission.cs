...
{
    /// <inheritdoc />
    public partial class ReplaceManageProcessesPermission : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                CROSS JOIN (
                    VALUES
                        (N'StartProcesses'),
                        (N'UpdateProcessSteps'),
                        (N'AssignProcessSteps'),
                        (N'CloseProcesses')
                ) AS replacement([Permission])
                WHERE legacyGrant.[Permission] = N'ManageProcesses'
                    AND NOT EXISTS (
                        SELECT 1
                ...
                WHERE [Permission] = N'ManageProcesses';
                ...
                WHERE legacyGrant.[Permission] = N'ManageRoles'
                    AND NOT EXISTS (
                        SELECT 1
                        FROM [PermissionGrants] AS existingGrant
                        WHERE existingGrant.[UserId] = legacyGrant.[UserId]
                ...
                WHERE rankedGrant.[GrantRank] = 1
                    AND NOT EXISTS (
                        ...
                        WHERE existingGrant.[UserId] = rankedGrant.[UserId]
                            AND existingGrant.[Permission] = N'ManageProcesses'
                    );
